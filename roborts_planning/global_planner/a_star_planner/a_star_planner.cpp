...
AStarPlanner::AStarPlanner(CostmapPtr costmap_ptr)
    ...
    std::string full_path =
        ...
        "/global_planner/a_star_planner/config/a_star_planner_config.prototxt";

    if (!roborts_common::ReadProtoFromTextFile(full_path.c_str(),
                                               &a_star_planner_config)) {
        RCLCPP_ERROR(AStarLogger(),
                     "Cannot load a star planner protobuf configuration file.");
    }
    ...
    goal_search_tolerance_ =
        a_star_planner_config.goal_search_tolerance() /
...
ErrorInfo AStarPlanner::Plan(const geometry_msgs::msg::PoseStamped &start,
    ...
    if (!costmap_ptr_->GetCostMap()->World2Map(start.pose.position.x,
                                               ...
                                               start_x,
                                               start_y)) {
        RCLCPP_WARN(AStarLogger(),
                    "Failed to transform start pose from map frame to costmap frame");
    ...
    if (!costmap_ptr_->GetCostMap()->World2Map(goal.pose.position.x,
                                               ...
                                               goal_y)) {
        RCLCPP_WARN(AStarLogger(),
                    "Failed to transform goal pose from map frame to costmap frame");
        return ErrorInfo(ErrorCode::GP_POSE_TRANSFORM_ERROR,
    ...
    if (costmap_ptr_->GetCostMap()->GetCost(goal_x, goal_y) <
        ...
        while (tmp_goal_y <= goal_y + goal_search_tolerance_) {
            ...
            while (tmp_goal_x <= goal_x + goal_search_tolerance_) {
                unsigned char cost =
                    costmap_ptr_->GetCostMap()->GetCost(tmp_goal_x,
                                                         tmp_goal_y);
                unsigned int dist =
                    abs(static_cast<int>(goal_x - tmp_goal_x)) +
    ...
    if (!goal_valid) {
        ...
        goal_index = costmap_ptr_->GetCostMap()->GetIndex(valid_goal[0],
                                                          valid_goal[1]);
...
ErrorInfo AStarPlanner::SearchPath(const int &start_index,
    ...
    gridmap_width_ = costmap_ptr_->GetCostMap()->GetSizeXCell();
    gridmap_height_ = costmap_ptr_->GetCostMap()->GetSizeYCell();
    RCLCPP_INFO(AStarLogger(), "Search in a map %d",
                gridmap_width_ * gridmap_height_);
    cost_ = costmap_ptr_->GetCostMap()->GetCharMap();
    ...
    int current_index = 0, move_cost = 0, h_score = 0, count = 0;
    ...
    while (!openlist.empty()) {
        ...
        if (current_index == goal_index) {
            RCLCPP_INFO(AStarLogger(), "Search takes %d cycle counts", count);
            break;
        }
        ...
        for (auto neighbor_index : neighbors_index) {
            ...
            if (g_score_.at(neighbor_index) >
                ...
                if (state_.at(neighbor_index) == SearchState::NOT_HANDLED) {
                    ...
                    f_score_.at(neighbor_index) =
                        g_score_.at(neighbor_index) + h_score;
    ...
    if (current_index != goal_index) {
        RCLCPP_WARN(AStarLogger(), "Global planner can't search the valid path!");
        return ErrorInfo(ErrorCode::GP_PATH_SEARCH_ERROR,
                         "Valid global path not found.");
    }

    unsigned int iter_index = current_index, iter_x = 0, iter_y = 0;

    geometry_msgs::msg::PoseStamped iter_pos;
    iter_pos.pose.orientation.w = 1;
    iter_pos.header.frame_id = "map";
    path.clear();
    costmap_ptr_->GetCostMap()->Index2Cells(iter_index, iter_x, iter_y);
    costmap_ptr_->GetCostMap()->Map2World(iter_x, iter_y,
                                          iter_pos.pose.position.x,
                                          iter_pos.pose.position.y);
...
ErrorInfo AStarPlanner::GetMoveCost(const int &current_index,
    ...
    if (abs(neighbor_index - current_index) == 1 ||
        abs(neighbor_index - current_index) == static_cast<int>(gridmap_width_)) {
        move_cost = 10;
    } else if (
        abs(neighbor_index - current_index) ==
            static_cast<int>(gridmap_width_ + 1) ||
        abs(neighbor_index - current_index) ==
            static_cast<int>(gridmap_width_ - 1)) {
        move_cost = 14;
    } else {
...
void AStarPlanner::GetManhattanDistance(const int &index1,
                                        const int &index2,
                                        int &manhattan_distance) const {
    manhattan_distance =
        heuristic_factor_ * 10 *
        (abs(static_cast<int>(index1 / gridmap_width_) -
             static_cast<int>(index2 / gridmap_width_)) +
         abs(static_cast<int>(index1 % gridmap_width_) -
             static_cast<int>(index2 % gridmap_width_)));
}
...
void AStarPlanner::GetNineNeighbors(const int &current_index,
    ...
    if (current_index - static_cast<int>(gridmap_width_) >= 0) {
        neighbors_index.push_back(current_index - gridmap_width_);
    }
    if (current_index - static_cast<int>(gridmap_width_) - 1 >= 0 &&
        (current_index - static_cast<int>(gridmap_width_) - 1 + 1) %
                static_cast<int>(gridmap_width_) !=
            0) {
        neighbors_index.push_back(current_index - gridmap_width_ - 1);
    }
    if (current_index - 1 >= 0 &&
        (current_index - 1 + 1) % static_cast<int>(gridmap_width_) != 0) {
        neighbors_index.push_back(current_index - 1);
    }
    if (current_index + static_cast<int>(gridmap_width_) - 1 <
            static_cast<int>(gridmap_width_ * gridmap_height_) &&
        (current_index + static_cast<int>(gridmap_width_) - 1 + 1) %
                static_cast<int>(gridmap_width_) !=
            0) {
        neighbors_index.push_back(current_index + gridmap_width_ - 1);
    }
    if (current_index + static_cast<int>(gridmap_width_) <
        static_cast<int>(gridmap_width_ * gridmap_height_)) {
        neighbors_index.push_back(current_index + gridmap_width_);
    }
    if (current_index + static_cast<int>(gridmap_width_) + 1 <
            static_cast<int>(gridmap_width_ * gridmap_height_) &&
        (current_index + static_cast<int>(gridmap_width_) + 1) %
                static_cast<int>(gridmap_width_) !=
            0) {
        neighbors_index.push_back(current_index + gridmap_width_ + 1);
    }
    if (current_index + 1 < static_cast<int>(gridmap_width_ * gridmap_height_) &&
        (current_index + 1) % static_cast<int>(gridmap_width_) != 0) {
        neighbors_index.push_back(current_index + 1);
    }
    if (current_index - static_cast<int>(gridmap_width_) + 1 >= 0 &&
        (current_index - static_cast<int>(gridmap_width_) + 1) %
