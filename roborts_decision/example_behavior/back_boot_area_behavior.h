...
class BackBootAreaBehavior {
 public:
  BackBootAreaBehavior(ChassisExecutor *&chassis_executor, Blackboard *&blackboard,
    ...
    if (!LoadParam(proto_file_path)) {
      RCLCPP_ERROR(rclcpp::get_logger("back_boot_area_behavior"), "%s can't open file",
                   __FUNCTION__);
    }
  }
  ...
  void Run() {
    ...
    if (executor_state != BehaviorState::RUNNING) {
      ...
      tf2::Quaternion rot1;
      tf2::Quaternion rot2;
      tf2::fromMsg(boot_position_.pose.orientation, rot1);
      tf2::fromMsg(robot_map_pose.pose.orientation, rot2);
  ...
  bool LoadParam(const std::string &proto_file_path) {
    ...
    boot_position_.pose.orientation = QuaternionFromRollPitchYaw(
 ...
 private:
  ...
  geometry_msgs::msg::PoseStamped boot_position_;
};
} // namespace roborts_decision
