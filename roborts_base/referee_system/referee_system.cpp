...
RefereeSystem::RefereeSystem(std::shared_ptr<roborts_sdk::Handle> handle, rclcpp::Node::SharedPtr node) :
...
void RefereeSystem::ROS_Init() {
  ros_game_status_pub_ = node_->create_publisher<roborts_msgs::msg::GameStatus>("game_status", 30);
  ros_game_result_pub_ = node_->create_publisher<roborts_msgs::msg::GameResult>("game_result", 30);
  ros_game_survival_pub_ = node_->create_publisher<roborts_msgs::msg::GameSurvivor>("game_survivor", 30);
  ros_bonus_status_pub_ = node_->create_publisher<roborts_msgs::msg::BonusStatus>("field_bonus_status", 30);
  ros_supplier_status_pub_ = node_->create_publisher<roborts_msgs::msg::SupplierStatus>("field_supplier_status", 30);
  ros_robot_status_pub_ = node_->create_publisher<roborts_msgs::msg::RobotStatus>("robot_status", 30);
  ros_robot_heat_pub_ = node_->create_publisher<roborts_msgs::msg::RobotHeat>("robot_heat", 30);
  ros_robot_bonus_pub_ = node_->create_publisher<roborts_msgs::msg::RobotBonus>("robot_bonus", 30);
  ros_robot_damage_pub_ = node_->create_publisher<roborts_msgs::msg::RobotDamage>("robot_damage", 30);
  ros_robot_shoot_pub_ = node_->create_publisher<roborts_msgs::msg::RobotShoot>("robot_shoot", 30);
  ros_sub_projectile_supply_ = node_->create_subscription<roborts_msgs::msg::ProjectileSupply>(
    "projectile_supply", 1, std::bind(&RefereeSystem::ProjectileSupplyCallback, this, std::placeholders::_1));
}
...
void RefereeSystem::GameSurvivorCallback(const std::shared_ptr<roborts_sdk::cmd_game_robot_survivors> raw_game_survivor){
  roborts_msgs::msg::GameSurvivor game_survivor;
...
void RefereeSystem::GameEventCallback(const std::shared_ptr<roborts_sdk::cmd_event_data> raw_game_event){
  roborts_msgs::msg::BonusStatus bonus_status;
...
void RefereeSystem::SupplierStatusCallback(const std::shared_ptr<roborts_sdk::cmd_supply_projectile_action> raw_supplier_status){
  roborts_msgs::msg::SupplierStatus supplier_status;
...
void RefereeSystem::RobotStatusCallback(const std::shared_ptr<roborts_sdk::cmd_game_robot_state> raw_robot_status){
  ...
  if(robot_id_ != raw_robot_status->robot_id){
    switch (raw_robot_status->robot_id){
      case 3: robot_status.id = 3; robot_id_ = 3; break;
      case 4: robot_status.id = 4; robot_id_ = 4; break;
      case 13: robot_status.id = 13; robot_id_ = 13; break;
      case 14: robot_status.id = 14; robot_id_ = 14; break;
      default:
        robot_status.id = raw_robot_status->robot_id;
        RCLCPP_ERROR(node_->get_logger(), "For AI challenge, please set robot id to Blue3/4 or Red3/4");
        return;
    }
  } else {
    robot_status.id = raw_robot_status->robot_id;
...
void RefereeSystem::RobotBonusCallback(const std::shared_ptr<roborts_sdk::cmd_buff_musk> raw_robot_bonus){
  roborts_msgs::msg::RobotBonus robot_bonus;
...
void RefereeSystem::ProjectileSupplyCallback(const roborts_msgs::msg::ProjectileSupply::SharedPtr projectile_supply){
  if (robot_id_ == 0xFF) {
    RCLCPP_ERROR(node_->get_logger(), "Can not get robot id before requesting for projectile supply.");
    return;
  }
